#' @title Process simulations using 1-compartment IV-bolus model with 1-compartment first-order formation metabolite model.
#' @description \code{OneCompIVbolusOneCompMetab} function accepts NONMEM-style data frame for one subject and calculates drug amount in the respective compartments.
#' The data frame should have the following columns: \code{ID, TIME, AMT, CL, V, CLM, VM, FR}.
#'
#' where:
#' \tabular{ll}{
...
#' }
#'
#' @usage OneCompIVbolusOneCompMetab(inputDataFrame)
#'
#' @param inputDataFrame which is the NONMEM-style data frame that contains columns for: \code{ID, TIME, AMT, CL, V, CLM, VM, FR}.\cr
...
#' @export
#------------------------------------------------------------------------------------
# IV bolus- 1 compartment parent with 1 compartment first-order metabolite formation
#------------------------------------------------------------------------------------
OneCompIVbolusOneCompMetab    <- function(inputDataFrame){
#Accepts a NONMEM style data frame for 1 subject with columns for TIME, AMT, MDV, CL, V, CLM, VM, FR
#Returns a dataframe with populated columns for A1,AM, IPREDP, IPREDM
    ...
    OneCompIVbolusOneCompMetabCpp ( inputDataFrame )
    ...
    inputDataFrame <- subset(inputDataFrame, select=-c(k10,CLpop1,CLpop2,kmf,kme))

    #Return output
    inputDataFrame
}

#' @title Process simulations using 2-compartment IV-bolus model with 1-compartment first-order formation metabolite model.
#' @description \code{TwoCompIVbolusOneCompMetab} function accepts NONMEM-style data frame for one subject and calculates drug amount in the respective compartments.
#' The data frame should have the following columns: \code{ID, TIME, AMT, CL, V1, Q2, V2, CLM, VM, FR}.
#'
#' where:
#' \tabular{ll}{
#' \code{ID}: \tab is the subject ID\cr
...
#' }
#'
#' @usage TwoCompIVbolusOneCompMetab(inputDataFrame)
#'
#' @param inputDataFrame which is the NONMEM-style data frame that contains columns for: \code{ID, TIME, AMT, CL, V1, Q2, V2, Q3, V3, CLM, VM, FR}.\cr
#' @return The function calculates the parent and metabolite amounts in the respective compartments of the pharmacokinetic model.
#' This includes the amount in the parent central (\code{A1} & individual predicted parent concentrations, \code{IPREDP})
#' parent peripheral \code{(A2)}, and metabolite central (\code{AM} & individual predicted metabolite concentrations, \code{IPREDM}) compartments.
#' The function returns the output added to the \code{inputDataFrame}
#'
#' @details
#' To simulate a population (i.e. the \code{inputDataFrame} has more than one subject \code{ID}),    the function
...
#' @export
#------------------------------------------------------------------------------------
# IV bolus- 2 compartment parent with 1 compartment first-order metabolite formation
#------------------------------------------------------------------------------------
TwoCompIVbolusOneCompMetab <- function(inputDataFrame){
#Accepts a NONMEM style data frame for 1 subject with columns for TIME, AMT,MDV, CL, V1, Q, V2,CLM, VM, FR
#Returns a dataframe with populated columns for A1, A2, AM, IPREDP, IPREDM
    ...
    TwoCompIVbolusOneCompMetabCpp( inputDataFrame )
    ...
    #subset unnecessary columns
    inputDataFrame <- subset(inputDataFrame, select=-c(k10,k12,k21,k20,CLpop1,CLpop2,kmf,kme))

    #Return output
    inputDataFrame
}

#' @title Process simulations using 3-compartment IV-bolus model with 1-compartment first-order formation metabolite model.
#' @description \code{ThreeCompIVbolusOneCompMetab} function accepts NONMEM-style data frame for one subject and calculates drug amount in the respective compartments.
#' The data frame should have the following columns: \code{ID, TIME, AMT, CL, V1, Q2, V2, Q3, V3, CLM, VM, FR}.
#'
#' where:
#' \tabular{ll}{
#' \code{ID}: \tab is the subject ID\cr
...
#' }
#'
#' @usage ThreeCompIVbolusOneCompMetab(inputDataFrame)
#'
#' @param inputDataFrame which is the NONMEM-style data frame that contains columns for: \code{ID, TIME, AMT, CL, V1, Q2, V2, Q3, V3, CLM, VM, FR}.\cr
#' @return The function calculates the parent and metabolite amounts in the respective compartments of the pharmacokinetic model.
#' This includes the amount in the parent central (\code{A1} & individual predicted parent concentrations, \code{IPREDP})
...
#' @export
#------------------------------------------------------------------------------------
# IV bolus- 3 compartment parent with 1 compartment first-order metabolite formation
#------------------------------------------------------------------------------------
ThreeCompIVbolusOneCompMetab <- function(inputDataFrame){
#Accepts a NONMEM style data frame for 1 subject with columns for TIME, AMT,MDV,CL, V1, Q2, V2, Q3, V3, CLM, VM, FR
#Returns a dataframe with populated columns for A1, A2, A3,AM, IPREDP, IPREDM
    ...
    #set initial values in the compartments
    inputDataFrame$A1[inputDataFrame$TIME==0] <- inputDataFrame$AMT[inputDataFrame$TIME==0]	# Parent amount in the central compartment at time zero.
    inputDataFrame$A2[inputDataFrame$TIME==0] <- 0											# Parent amount in the 1st-peripheral compartment at time zero.
    inputDataFrame$A3[inputDataFrame$TIME==0] <- 0											# Parent amount in the 2nd-peripheral compartment at time zero.
    inputDataFrame$AM[inputDataFrame$TIME==0] <- 0											# Metabolite amount at time zero.

    ThreeCompIVbolusOneCompMetabCpp( inputDataFrame )
    ...
    #subset unnecessary columns
    inputDataFrame <- subset(inputDataFrame, select=-c(k10,k12,k21,k20,k13,k31,k30,CLpop1,CLpop2,kmf,kme))
